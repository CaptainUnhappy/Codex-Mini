...
Codex Mini 采用本机优先的架构。Codex Desktop 的登录状态、线程切换、输入发送、回复读取，以及手机网页使用的本地访问令牌，均由用户 Mac 上的本机服务管理。访问链接和 token 属于用户本机入口凭据，请妥善保管。

开启 Pro 外网入口后，A1 服务器仅作为远程连接通道和授权服务节点，用于：

- 将手机端请求转发到对应用户自己的 Mac
- 校验 Pro 授权状态，并根据设备 ID 匹配连接
- 维护在线状态、心跳和基础连接状态
- 统计套餐流量、图片次数和请求大小，用于限流与服务稳定性保障

聊天内容、图片内容和 Codex 会话数据仍由用户自己的 Mac 与 Codex Desktop 处理；A1 中转层只承担连接、授权、在线状态和流量统计相关职责。
